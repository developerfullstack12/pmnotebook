    ...
    if (!class_exists('myCRED_Hook_Learndash_Completing_Quiz') && class_exists('myCRED_Hook')) {
         ...
         class myCRED_Hook_Learndash_Completing_Quiz extends myCRED_Hook {
            ...
            public function run() {
                ...
                add_action('learndash_quiz_completed', array($this, 'quiz_completed'), 40, 2);
            ...
            public function quiz_completed($quiz_data, $current_user) {
              ...
              $tags = get_terms([
               ...
               if(  isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_quiz_completed']['select_quiz'] ) && in_array( $quiz_data['quiz']->ID, $prefs['specific_quiz_completed']['select_quiz'] )  ) {

                  $hook_index = array_search( $quiz_data['quiz']->ID, $prefs['specific_quiz_completed']['select_quiz'] );
                     ...
                     if (  
                        ...
                        if ( $this->core->has_entry( 'learndash_quiz_complete', $quiz_data['quiz']->ID, $current_user->ID) ) return;
                ...
                elseif ( isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_quiz_completed']['select_quiz'] ) && in_array( 'tags' , $prefs['specific_quiz_completed']['select_option'] ) && !empty( $prefs['specific_quiz_completed']['creds'][$hook_index] ) ) {
                    ...
                    foreach ( $quiz_post as $quizzes ) {
                            if ( $quizzes->ID == $quiz_data['quiz']->ID && $quiz_data['pass'] ) {
                                 $this->core->add_creds(
                                'learndash_quiz_complete',
                                $current_user->ID,
                                $prefs['specific_quiz_completed']['creds'][$hook_index],
                  ...
                  elseif ( isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_quiz_completed']['select_quiz'] ) && in_array( 0 , $prefs['specific_quiz_completed']['select_quiz'] )  ) {
                       ...
                       if ( $hook_index === false ) {
                        ...
                        foreach ( $this->prefs['specific_quiz_completed']['select_quiz'] as $key => $value ) {
                            ...
                            if( $this->prefs['specific_quiz_completed']['select_quiz'][$key] == $quiz_data['quiz']->ID && $value == 0 ) {
                                $hook_index = $key;
                            }
                        }
                    }
                    ...
                    $terms_ids = $this->mycred_ld_get_term_ids( $quiz_id, 'ld_quiz_tag' );
                       ...
                       if ($this->over_hook_limit('', 'learndash_quiz_complete', $current_user->ID))
                        return;
            ...
            public function specific_field_name( $field = '' ) {
              ...
              public function mycred_learndash_quiz_arrange_data( $specific_quiz_hook_data ){
                ...
                foreach ( $specific_quiz_hook_data['creds'] as $key => $value ) {
                    ...
                    $hook_data[$key]['limit'] = $specific_quiz_hook_data['limit'][$key];
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control  mycred-learndash-quiz-options" id="user_selected" name="<?php echo $this->specific_field_name(array('specific_quiz_completed' => 'select_option')); ?>" value=""  >
                        ...
                        <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        <label><?php _e( 'User Selected Option', 'select_quiz' ); ?></label>
                                        <select class="form-control user_select_quiz mycred-learndash-quiz " id="selected_option"  name="<?php echo $this->specific_field_name(array('specific_quiz_completed' => 'select_quiz')); ?>" value=""  >
                                        ...
                                        if($label['select_option'] == 'quiz') {
                                            ...
                                            $quiz_id = $quiz_name->ID;
                                                ...
                                                echo '<option class="select-value" value="'.$quiz_name->ID.'" '. ( $quiz_name->ID == $label['select_quiz'] ? ' selected' : '') .' >'.$quiz_name->post_title.'</option>';
                                            }
                                        ...
                                        foreach($tags as $tag) {
                                            ...
                                            $quiz_field = $this->quiz_name();
                                             ...
                                             echo '<option class="select-value" value="'.$tag->term_id.'" '. ( $tag->term_id == $label['select_quiz'] ? ' selected' : '') .' >'.$tag->name.'</option>';
                        ...
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                            <div class="form-group">
                                <label for="<?php echo $this->field_id(array('specific_quiz_completed' => 'creds')); ?>"><?php echo $this->core->plural(); ?></label>
                                <input type="text" name="<?php echo $this->specific_field_name(array('specific_quiz_completed' => 'creds')); ?>" id="<?php echo $this->field_id(array('specific_quiz_completed' => 'creds')); ?>" value="<?php echo $this->core->number( $label['creds']); ?>" class="form-control mycred-learndash-quiz-creds" />
                            </div>
                        </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                            <div class="form-group">
                                <label for="<?php echo $this->field_id(array('specific_quiz_completed' => 'log')); ?>"><?php _e('Log Template', 'mycred'); ?></label>
                                <input type="text" name="<?php echo $this->specific_field_name(array('specific_quiz_completed' => 'log')); ?>" id="<?php echo $this->field_id(array('specific_quiz_completed' => 'log')); ?>" value="<?php echo esc_attr($label['log']) ; ?>" class="form-control mycred-learndash-quiz-log" />
                                <span class="description"><?php echo $this->available_template_tags(array('general', 'post')); ?></span>
                            </div>
                        </div>
