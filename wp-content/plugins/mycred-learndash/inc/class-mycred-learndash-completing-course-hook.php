                ...
                elseif ( isset($prefs['check_specific_hook']) && $prefs['check_specific_hook'] == '1' && !empty( $prefs['specific_course_completed']['select_course'] )  && in_array( 0 , $prefs['specific_course_completed']['select_course'] ) ) {
                    ...
                    $hook_index = array_search( $args['course']->ID, $prefs['specific_course_completed']['select_course'] );
                       ...
                       if ($this->over_hook_limit('specific_course_completed', 'learndash_course_complete', $args['user']->ID))
                        ...
                        echo '<option class="select-value" value="'.$tag->term_id.'" '. ( $tag->term_id == $prefs['specific_course_completed']['select_course'] ? ' selected' : '') .' >'.$tag->name.'</option>';
                  ...
                  elseif(isset($prefs['check_specific_hook']) && ! $prefs['check_specific_hook'] == '1') {
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control  mycred-learndash-options" id="user_selected" name="<?php echo $this->specific_field_name(array('specific_course_completed' => 'select_option')); ?>" value=""  >
                                            <?php
                                ...
                                <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        <label><?php _e( 'User Selected Option', 'select_course' ); ?></label>
                                        <select class="form-control user_select_course " id="selected_option"  name="<?php echo $this->specific_field_name(array('specific_course_completed' => 'select_course')); ?>" value=""  >
                                        <?php 
                                        $selected = '';
                                        if($label['select_option'] == 'course') {
                                            foreach ($course_field as $course_name) {
                                            $course_id = $course_name->ID;
                                                echo '<option class="select-value" value="'.$course_name->ID.'" '. ( $course_name->ID == $label['select_course'] ? ' selected' : '') .' >'.$course_name->post_title.'</option>';
                                            }
                                        ...
                                        foreach($tags as $tag) {
                                            $course_field = $this->course_name();
                                             echo '<option class="select-value" value="'.$tag->term_id.'" '. ( $tag->term_id == $label['select_course'] ? ' selected' : '') .' >'.$tag->name.'</option>';
                                        }
                                    }
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        <label for="<?php echo $this->field_id(array('specific_course_completed' => 'creds')); ?>"><?php echo $this->core->plural(); ?></label>
                                        <input type="text" name="<?php echo $this->specific_field_name(array('specific_course_completed' => 'creds')); ?>" id="<?php echo $this->field_id(array('specific_course_completed' => 'creds')); ?>" value="<?php echo $this->core->number( $label['creds']); ?>" class="form-control mycred-learndash-course-creds" />
                                    </div>
                                </div>
                                <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                                    <div class="form-group">
                                        <label for="<?php echo $this->field_id(array('specific_course_completed' => 'log')); ?>"><?php _e('Log Template', 'mycred'); ?></label>
                                        <input type="text" name="<?php echo $this->specific_field_name(array('specific_course_completed' => 'log')); ?>" id="<?php echo $this->field_id(array('specific_course_completed' => 'log')); ?>" value="<?php echo esc_attr($label['log']) ; ?>" class="form-control mycred-learndash-course-log" />
                                        <span class="description"><?php echo $this->available_template_tags(array('general', 'post')); ?></span>
                                    </div>
                                </div>
                            </div>
