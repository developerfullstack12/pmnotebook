...
class mycred_fortunewheel_Statistics {
    ...
    function __construct() {
        ...
        add_action( 'views_edit-fortunewheel-stats', array($this,'mycred_progress_stats') );
    }

    function mycred_progress_stats() {
    ...
    function mycred_fortunewheel_register_statistics() {
        ...
        $labels = array(
            'name'               => _x( 'Wheel Stats', 'post type general name', 'fortunewheel' ),
            'singular_name'      => _x( 'Wheel Stats', 'post type singular name', 'fortunewheel' ),
            'menu_name'          => _x( 'Wheel Stats', 'admin menu', 'fortunewheel' ),
            'name_admin_bar'     => _x( 'Wheel Stats', 'add new on admin bar', 'fortunewheel' ),
            'add_new'            => _x( 'Add New', 'Wheel Stats', 'fortunewheel' ),
            'add_new_item'       => __( 'Add New Wheel Stats', 'fortunewheel' ),
            'new_item'           => __( 'New Wheel Stats', 'fortunewheel' ),
            'edit_item'          => __( 'Edit Wheel Stats', 'fortunewheel' ),
            'view_item'          => __( 'View Wheel Stats', 'fortunewheel' ),
            'all_items'          => __( 'All Wheel Stats', 'fortunewheel' ),
            'search_items'       => __( 'Search Wheel Stats', 'fortunewheel' ),
            'parent_item_colon'  => __( 'Parent Wheel Stats:', 'fortunewheel' ),
            'not_found'          => __( 'No Wheel Stats found Please Select another wheel from dropdown to display the result.', 'fortunewheel' ),
            'not_found_in_trash' => __( 'No Wheel Stats found in Trash.', 'fortunewheel' )
        );
    ...
    function mycred_fortunewheel_statistics_callback() {
        ...
        $wheel__id = str_replace( '_','', $_POST['wheel_id']) ;

        if( $fortunewheel_protect_post->mycred_fortunewheel_free_played($wheel__id) >= mycred_fortunewheel_get_post_meta( $wheel__id,'fortunewheel_free_attempts') && empty( $paid_spin ) ) {
            $html = '<div class="fortunewheel-cheat-msg">';
            $html .= apply_filters('mycred_fortunewheel_spining_limit_exceed_text','Spin Limit Exceeded - STOP 😕');
            $html .= '</div>';
        ...
        // Now User can play spin
        update_user_meta($get_user_id,'wheel_spining',0);
        update_user_meta($get_user_id,'paid_spin',0);

        echo 'FREE PLAYED' . $fortunewheel_protect_post->mycred_fortunewheel_free_played($wheel__id);
        echo 'free_attempts' . mycred_fortunewheel_get_post_meta( $wheel__id,'fortunewheel_free_attempts');
        echo 'DONE ID:' . $get_user_id;
        die();
    }
    ...
    function mycred_fortunewheel_stats_menu() {
        ...
        add_submenu_page( 'edit.php?post_type=mycred_fortunewheels', 'Wheel Stats', 'Wheel Stats',
            'manage_options', 'edit.php?post_type=fortunewheel-stats');
    }
