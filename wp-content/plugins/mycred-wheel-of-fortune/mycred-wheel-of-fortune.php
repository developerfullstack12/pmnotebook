...
define('MYCRED_WOF_SLUG', 'mycred-wheel-of-fortune');
define('MYCRED_WOF_VERSION', '1.5.3');
...
// add_filter( 'views_edit-fortunewheel-stats', 'mycred_fortunewheel_export_list_btn' );
function mycred_fortunewheel_export_list_btn( $views ) {
...
function mycred_fortunewheel_admin_wheel_script() {
    ...
    if( get_post_type() == 'fortunewheel-stats') {
        wp_enqueue_style( 'fortunewheel-stats-style', mycred_fortunewheel_PLUGIN_URL . 'assets/css/stats-style.css' );
    }
        $ajaxurl = array(
...
function mycred_fortunewheel_activation( $plugin, $network_activation ) {
    ...
    if( $plugin == plugin_basename( __FILE__ ) ) {
        ...
        if (!is_plugin_active('mycred/mycred.php') ) {
            deactivate_plugins(plugin_basename(__FILE__));
            wp_die( 'It is required to install <a href="https://wordpress.org/plugins/mycred/" target="_blank">myCRED</a> plugin before using this plugin.' );
        } else {
