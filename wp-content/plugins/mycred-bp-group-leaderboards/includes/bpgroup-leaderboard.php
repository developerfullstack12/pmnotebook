...
<?php
		...
		public function custom_leaderboard_fields( $leaderboard_type, $group_id = false ) {
			...
			if ( ! is_array( $group_id ) )
				...
				$setup = $group_id;
			$from="bp_group_leaderboard_from";
			$until="bp_group_leaderboard_until";

			if( is_admin() ){
					
					$from="mycred_bp_leaderboards[from]";
					$until="mycred_bp_leaderboards[until]";	

			}

?>
<div id="leaderboard-timeframe" style="display: <?php if ( $leaderboard_type == 'custom' ) echo 'block'; else echo 'none'; ?>;">
	<p><label for="bp-group-leaderboard-from"><?php _e( 'From Date:', 'bpmycredleaderboard' ); ?></label><input type="date" placeholder="YYYY-MM-DD" name="<?php echo $from; ?>" id="bp-group-leaderboard-from" value="<?php echo esc_attr( $setup['from'] ); ?>" /></p>
	<p><label for="bp-group-leaderboard-until"><?php _e( 'Until Date:', 'bpmycredleaderboard' ); ?></label><input type="date" placeholder="YYYY-MM-DD" name="<?php echo $until; ?>" id="bp-group-leaderboard-until" value="<?php echo esc_attr( $setup['until'] ); ?>" /></p>
</div>
