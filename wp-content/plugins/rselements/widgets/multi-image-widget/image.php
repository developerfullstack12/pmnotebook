...
class Rsaddon_pro_Mulimage_hover_Widget extends \Elementor\Widget_Base {
    /**
     * Get widget name.
     *
    ...
    public function get_title() {
        return esc_html__( 'RS Multi Image Parallax', 'rsaddon' );
    }
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'image',
			[
				'label' => __( 'Choose Parallax Image', 'elementor' ),
		...
		$this->add_control(
			'2ndimage',
			[
				'label' => __( 'Choose Rotating Image', 'elementor' ),
				'type' => Controls_Manager::MEDIA,
				'dynamic' => [
					'active' => true,
				],
		...
		$this->add_responsive_control(
            'animation_image_horaizontal',
            [
                'label' => esc_html__( 'Animation Image Position Horaizontal', 'rsaddon' ),
                'type' => Controls_Manager::SLIDER,
                'size_units' => [ '%' ],
                'range' => [
                    '%' => [
                ...
                'selectors' => [
                    '{{WRAPPER}} .rs-multi-rallax .rs-multi-image' => 'left: {{SIZE}}%;',                   
                ],
            ]
        );
		$this->add_responsive_control(
            'animation_image_vertical',
            [
                'label' => esc_html__( 'Animation Image Position Vertical', 'rsaddon' ),
                'type' => Controls_Manager::SLIDER,
                'size_units' => [ '%' ],
                'range' => [
                    '%' => [
                ...
                'selectors' => [
                    '{{WRAPPER}} .rs-multi-rallax .rs-multi-image' => 'top: {{SIZE}}%;',                   
                ],
            ]
        );
        $this->add_control(
			'animation_opacity_hover',
			[
				'label' => __( 'Animation Image Opacity', 'elementor' ),
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'max' => 1,
				...
				'selectors' => [
					'{{WRAPPER}} .rs-multi-rallax .rs-multi-image' => 'opacity: {{SIZE}};',
				],
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'rs-multi-rallax elementor-image' );
...
?>
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<?php if(!empty($settings['2ndimage']['url'])) : ?>
                <img class="rs-multi-image animated rotate infinite" src="<?php echo esc_url($settings['2ndimage']['url']);?>" alt="image"/>
            <?php endif; ?>
